...
def handle_delete():
    """Removes a row record packet using its auto-assigned Row Index number."""
    del_idx_str = entry_delete.get().strip()
    try:
        idx = int(del_idx_str)
        # Verify row exists within the boundary limits of our live 2D array
        if 0 <= idx < len(active_hires):
            active_hires.pop(idx)
            refresh_table_display()
            entry_delete.delete(0, tk.END)
            messagebox.showinfo("Success", f"Row #{idx} removed successfully.")
        else:
